...
# Only increment versionCode if package version changed
if [ "$CURRENT_PACKAGE" != "$NEW_PACKAGE_VERSION" ]; then
    android_versions=$(derive_android_versions "$NEW_PACKAGE_VERSION" "$CURRENT_ANDROID_VERSION_CODE")
    NEW_ANDROID_VERSION_CODE=$(echo "$android_versions" | cut -d' ' -f1)
    NEW_ANDROID_VERSION_NAME=$(echo "$android_versions" | cut -d' ' -f2)
else
    # Keep current version code if package version didn't change
...
if [ "$CHANGELOG_UPDATE_NEEDED" = "true" ]; then
    ...
    echo -e "${WHITE}The CHANGELOG.md has been updated with version changes.${NC}"
...
if sed -i.tmp \
    ...
    -e "s/com\.emarsys:emarsys-firebase:$current_android_sdk_escaped\"/com.emarsys:emarsys-firebase:$FINAL_ANDROID_SDK_VERSION\"/" \
